...
| Stolen client secret | Attacker obtains `X-Client-Secret`. | Secret is returned once, stored as digest, and can be rotated through an authenticated one-time replacement endpoint. | Existing credentials authenticate until rotated; after rotation the old secret returns HTTP 401 and the audit trail records the action. |
...
| Signed webhook tampering | Intermediary changes webhook payload or replays modified body. | HMAC-SHA256 signature over timestamp plus canonical payload; event idempotency key persisted; webhook signing secrets can be rotated for new deliveries. | Partner should reject payloads whose computed signature does not match delivery signature. Existing deliveries keep the signature created with their original secret. |
...
- Webhook signing secrets are returned once, stored encrypted, and rotatable for new deliveries; hosted deployments still need tenant-specific webhook allowlists.
